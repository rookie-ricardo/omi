...
function resolveAlignClass(anchorX: AnchorX = "center", anchorY: AnchorY = "top"): string {
  const x =
    anchorX === "left" ? "justify-start" : anchorX === "right" ? "justify-end" : "justify-center";
...
export function Island({
  ...
  React.useEffect(() => {
    ...
    return bridge.registerLayer({
      ...
      canBack: dialogBehavior === "back-or-close" ? () => Boolean(onRequestBack) : undefined,
      back:
        dialogBehavior === "back-or-close"
  ...
  const hasEntryScale =
    transitionConfig?.entryStartScale != null && Math.abs(cfg.entryStartScale - 1) > 0.0001;
  const hasReplayEntryRequest = replayEntryKey != null;
  const shouldAnimateFromHiddenOnMount =
    shouldMorph || hasEntryTranslation || hasEntryScale || hasReplayEntryRequest;
  ...
  const shouldHideForWarmup =
    shouldMorph && isVisible && !isMorphWarmReady && !hasRenderedVisibleRef.current;
  ...
  const shellTransition = React.useMemo(
    () => (isPreShowWarmup ? { type: "tween" as const, duration: 0 } : layoutTransition),
