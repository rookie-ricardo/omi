 ...
 *
 * Storage locations:
 * - App override:   app settings store
 * - Preset themes:  bundled theme presets
 */
...
function mergeThemes(
  ...
  if (override.backgroundImage !== undefined) result.backgroundImage = override.backgroundImage;
...
export function composeThemes(...themes: Array<ThemeOverrides | undefined>): ThemeOverrides {
  return themes.reduce<ThemeOverrides>(
    (resolved, nextTheme) => mergeThemes(resolved, nextTheme),
    {},
  );
}
 ...
 */
export function themeToCSS(theme: ThemeOverrides, isDark = false): string {
  const vars = themeToVariables(theme, isDark);
  return Object.entries(vars)
    .map(([name, value]) => `${name}: ${value};`)
    .join("\n  ");
}

export function themeToVariables(theme: ThemeOverrides, isDark = false): Record<string, string> {
  ...
  if (colors.accent) {
    ...
    if (rgbValues) {
      vars["--accent-rgb"] = rgbValues;
    }
  }
  if (colors.info) vars["--info"] = colors.info;
  if (colors.success) vars["--success"] = colors.success;
  if (colors.destructive) vars["--destructive"] = colors.destructive;
  ...
  vars["--paper"] = colors.paper || bg;
  vars["--navigator"] = colors.navigator || bg;
  vars["--input"] = colors.input || bg;
  vars["--popover"] = colors.popover || bg;
  ...
  vars["--popover-solid"] = colors.popoverSolid || colors.popover || bg;
 ...
 */
export function getShikiTheme(shikiConfig: ShikiThemeConfig | undefined, isDark: boolean): string {
