...
module.exports = {
  meta: {
    ...
    docs: {
      description:
        "Disallow floating z-index tokens in island components. Use island-specific z-index tokens.",
  ...
  create(context) {
    const filename = String(context.getFilename?.() ?? "")
      .replace(/\\/g, "/")
      .toLowerCase();
    const isIslandContext =
      /\/components\/(annotations\/annotationislandmenu|overlay\/annotatablemarkdowndocument|ui\/island|ui\/islandfollowupcontentview)\.tsx$/.test(
        filename,
      );
    ...
    function isDisallowedFloatingToken(value) {
      ...
      return (
        normalized.includes("var(--z-floating-menu") ||
        normalized.includes("var(--z-floating-backdrop")
      );
    }
