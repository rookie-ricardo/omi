...
import { chmodSync, existsSync, mkdirSync, readdirSync, renameSync, rmSync, createWriteStream } from "node:fs";
...
// TODO: Replace with proper settings directory when Settings system is implemented (task #4)
function getBinDir(): string {
  return join(tmpdir(), "omi", "bin");
}
...
function isOfflineModeEnabled(): boolean {
  const value = process.env.OMI_OFFLINE;
