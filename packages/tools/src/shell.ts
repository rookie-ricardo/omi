...
function findBashOnPath(): string | null {
  if (process.platform === "win32") {
    ...
    try {
      const result = spawnSync("where", ["bash.exe"], { encoding: "utf8", timeout: 5000 });
  ...
  try {
    const result = spawnSync("which", ["bash"], { encoding: "utf8", timeout: 5000 });
    if (result.status === 0 && result.stdout) {
      const firstMatch = result.stdout.trim().split(/\r?\n/)[0];
      if (firstMatch) {
        return firstMatch;
...
export function getShellConfig(): { shell: string; args: string[] } {
  ...
  // Try to load custom shell path from settings
  ...
  // Check user-specified shell path
  if (customShellPath) {
    ...
    throw new Error(
      `Custom shell path not found: ${customShellPath}\nPlease update shellPath in ${getAgentDir()}/settings.json`,
    );
  }
  ...
  if (process.platform === "win32") {
    ...
    throw new Error(
      `No bash shell found. Options:\n` +
        ...
        `  3. Set shellPath in ${getAgentDir()}/settings.json\n\n` +
