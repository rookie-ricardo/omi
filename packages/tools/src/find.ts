...
export const findSchema: TSchema = Type.Object({
	pattern: Type.String({
...
/**
 * Create local find operations using node fs primitives.
 */
export function createLocalFindOperations(): FindOperations {
	return {
		exists: existsSync,
			...
			return new Promise((resolve, reject) => {
				...
				(async () => {
					try {
						...
						const ops = customOps ?? createLocalFindOperations();
						...
						// Default: try fd, fall back to Node.js glob
						const fdPath = await ensureTool("fd", true);
						if (!fdPath) {
							// Fallback: use Node.js glob module
							signal?.removeEventListener("abort", onAbort);

							let globResults: string[] = [];
							try {
								globResults = globSync(pattern, {
									cwd: searchPath,
									dot: true,
									ignore: ["**/node_modules/**", "**/.git/**"],
									absolute: false,
							...
							for (const entry of globResults) {
								relativized.push(toPosixPath(entry));
							}

							const resultLimitReached = relativized.length >= effectiveLimit;
							const sliced = resultLimitReached ? relativized.slice(0, effectiveLimit) : relativized;
							const rawOutput = sliced.join("\n");
							const truncation = truncateHead(rawOutput, { maxLines: Number.MAX_SAFE_INTEGER });
